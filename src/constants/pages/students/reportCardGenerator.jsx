...
const ReportCardGenerator = () => {
  ...
  const fetchClasses = async () => {
    try {
      const response = await API.get("/api/reports/classes");
  ...
  const fetchBatches = async (classNumber) => {
    try {
      const response = await API.get(
        `/api/reports/classes/${classNumber}/batches`
      );
  ...
  const fetchStudents = async (classNumber, batch) => {
    try {
      const response = await API.get(
        "/api/reports/students-by-class-batch",
        {
          params: {
            class: classNumber,
            batch: batch,
  ...
  const handleViewReportCard = (admissionNo) => {
    navigate(`/reports/report-card/${admissionNo}`);
  };
