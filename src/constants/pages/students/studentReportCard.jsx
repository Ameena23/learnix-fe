...
export default function StudentReportCard() {
  ...
  useEffect(() => {
    async function fetchReport() {
      try {
        const res = await API.get("/api/reports/report-card/data", {
  ...
  async function handleSaveRemark() {
    ...
    try {
      ...
      await API.post("/api/reports/term-remark", {
        admission_no: admission_no,
        term: term,
        year: studentInfo.year,
        remark: termRemark,
        created_by: localStorage.getItem("userRole") || "admin",
      });
  ...
  async function handleDownloadPDF() {
    try {
      const response = await API.get("/api/reports/report-card/pdf", {
        params: { admission_no, term },
        responseType: "blob",
      });
      ...
      link.href = url;
      link.setAttribute("download", `ReportCard_${admission_no}_${term}.pdf`);
      document.body.appendChild(link);
      link.click();
      link.remove();
    } catch (err) {
      console.error("PDF Download failed", err);
      alert("Failed to download PDF. Please check your permissions.");
    }
  }
    ...
    <MainLayout>
      <div style={styles.page}>
        ...
        <div style={styles.actionRow}>
          <button style={styles.backBtn} onClick={() => navigate("/studentss")}>
