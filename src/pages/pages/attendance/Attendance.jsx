...
export default function Attendance() {
  ...
  useEffect(() => {
    API.get("/api/filters/classes").then(res => {
    ...
    API
      ...
      .then(async (res) => {
        if (res.data.success) {
          ...
          const studentRes = await API.get("/api/students/students", {
            params: { class_number: classNum, batch },
          });
  ...
  const updateAttendance = async () => {
    try {
      setSaving(true);
      await API.put(`/api/attendance/update/${sessionId}`, {
        records: students,
      });
  ...
  const deleteAttendance = async () => {
    ...
    await API.delete(`/api/attendance/delete/${sessionId}`);
      ...
      <div style={styles.page}>
        ...
        <div style={styles.card}>
          ...
          <div style={{ textAlign: "center", marginTop: 20, display: "flex", gap: 12, justifyContent: "center" }}>
            {!isExisting && hasPermission('attendance', 'create') && (
